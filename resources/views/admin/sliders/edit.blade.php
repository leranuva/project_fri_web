    ...
    <x-ui.container>
        <x-ui.card glassmorphism="true" padding="lg" class="mb-6">
            ...
            <form method="POST" action="{{ route('admin.sliders.update', $slider) }}" enctype="multipart/form-data" class="space-y-6">
                    ...
                    @if($slider->image)
                        <div class="mt-4">
                            ...
                            <img 
                                ...
                                onerror="this.onerror=null; this.src='data:image/svg+xml,%3Csvg xmlns=%22http://www.w3.org/2000/svg%22 width=%22400%22 height=%22200%22%3E%3Crect fill=%22%23ffffff20%22 width=%22400%22 height=%22200%22/%3E%3Ctext fill=%22%23ffffff60%22 x=%2250%25%22 y=%2250%25%22 text-anchor=%22middle%22 dominant-baseline=%22middle%22%3EImagen no disponible%3C/text%3E%3C/svg%3E';"
                            >
                        </div>
                    @endif
                ...
                {{-- Video (opcional, alternativa a imagen) --}}
                <div class="p-4 rounded-xl bg-white/5 border border-white/10">
                    <x-input-label :value="__('Video del Slide (opcional)')" class="text-white/90 mb-2" />
                    <p class="text-xs text-white/60 mb-4">Si subes un video, se mostrará en lugar de la imagen. La imagen puede usarse como poster/fondo de respaldo.</p>
                    
                    <div class="mb-4 flex gap-4">
                        <label class="auth-form-checkbox cursor-pointer">
                            <input type="radio" name="video_source" value="url" id="video_source_url" {{ old('video_source', ($slider->video && str_starts_with($slider->video, 'http')) ? 'url' : 'url') === 'url' ? 'checked' : '' }} onchange="toggleVideoSource()">
                            <span class="text-white/90">Usar URL</span>
                        </label>
                        <label class="auth-form-checkbox cursor-pointer">
                            <input type="radio" name="video_source" value="file" id="video_source_file" {{ old('video_source', 'url') === 'file' ? 'checked' : '' }} onchange="toggleVideoSource()">
                            <span class="text-white/90">Subir desde PC</span>
                        </label>
                    </div>

                    <div id="video_url_field" style="display: {{ old('video_source', 'url') === 'url' ? 'block' : 'none' }};">
                        <x-text-input id="video_url" name="video_url" type="text" class="auth-form-input" :value="old('video_url', str_starts_with($slider->video ?? '', 'http') ? $slider->video : '')" placeholder="https://www.youtube.com/watch?v=..." />
                        <p class="mt-1 text-xs text-white/60">URL directa (MP4, WebM, OGG) o enlace de YouTube (youtube.com/watch?v=... o youtu.be/...)</p>
                    </div>

                    <div id="video_file_field" style="display: {{ old('video_source', 'url') === 'file' ? 'block' : 'none' }};">
                        <input type="file" id="video_file" name="video_file" accept="video/mp4,video/webm,video/ogg,video/quicktime" class="auth-form-input">
                        <p class="mt-1 text-xs text-white/60">Formatos: MP4, WebM, OGG, MOV (máx. 50MB)</p>
                    </div>

                    @if($slider->video)
                        <div class="mt-4">
                            <p class="text-sm text-white/70 mb-2">Video actual:</p>
                            @if(\App\Models\Slider::isYouTubeUrl($slider->video))
                                <iframe src="{{ \App\Models\Slider::getYouTubeEmbedUrl($slider->video) }}" class="w-full max-w-md rounded-lg border border-white/20" style="height: 200px;" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
                            @else
                                <video src="{{ $slider->video }}" controls class="w-full max-w-md rounded-lg border border-white/20" style="max-height: 200px;"></video>
                            @endif
                        </div>
                    @endif
                ...
                <script>
                    function toggleVideoSource() {
                        ...
                        if (urlRadio.checked) {
                            ...
                            if (urlInput) { urlInput.value = ''; urlInput.setAttribute('disabled', 'disabled'); }
                        }
                    }
                    document.addEventListener('DOMContentLoaded', function() { toggleVideoSource(); });
                </script>

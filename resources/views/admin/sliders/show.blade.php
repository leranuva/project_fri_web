    ...
    <x-ui.container>
        <x-ui.card glassmorphism="true" padding="lg" class="mb-6">
            ...
            <div class="space-y-6">
                    ...
                    <div class="relative w-full h-64 sm:h-96 rounded-lg overflow-hidden border border-white/20">
                        @if($slider->video)
                            @if(\App\Models\Slider::isYouTubeUrl($slider->video))
                                <iframe src="{{ \App\Models\Slider::getYouTubeEmbedUrl($slider->video) }}" class="absolute inset-0 w-full h-full object-cover" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
                            @else
                                <video src="{{ $slider->video }}" poster="{{ $slider->image }}" autoplay muted loop playsinline class="w-full h-full object-cover"></video>
                            @endif
                        @elseif($slider->image)
                        ...
                        @else
                            <div class="w-full h-full bg-white/10 flex items-center justify-center">
                                <span class="text-white/50">Sin imagen ni video</span>
                            </div>
                        @endif
                ...
                {{-- Información de Imagen y Video --}}
                <div class="p-6 rounded-xl bg-white/5 border border-white/10">
                    <h3 class="text-lg font-semibold text-white mb-4">Imagen y Video</h3>
                    <div class="space-y-4">
                        @if($slider->image)
                            <div>
                        ...
                        @if($slider->video)
                            <div>
                                <p class="text-sm text-white/70 mb-2">URL del Video</p>
                                <a href="{{ $slider->video }}" target="_blank" class="text-base font-medium text-blue-300 hover:text-blue-200 break-all">
                                    {{ $slider->video }}
                                </a>
                            </div>
                        @endif
                        @if(!$slider->image && !$slider->video)
                            <p class="text-white/60">Sin imagen ni video configurado</p>
                        @endif
                    </div>
                </div>

<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-lg sm:text-xl text-white leading-tight">Editar Página SEO</h2>
    </x-slot>
            ...
            <form method="POST" action="{{ route('admin.seo-pages.update', $seoPage) }}">
                ...
                <div class="space-y-4">
                    <x-ui.form-group name="keyword">
                        ...
                        <input type="text" name="keyword" value="{{ old('keyword', $seoPage->keyword) }}" class="auth-form-input w-full" required>
                    </x-ui.form-group>
                    <x-ui.form-group name="slug">
                        <x-slot name="labelSlot"><label class="text-white/90">Slug (URL)</label></x-slot>
                        <input type="text" name="slug" value="{{ old('slug', $seoPage->slug) }}" class="auth-form-input w-full" required>
                    </x-ui.form-group>
                    <x-ui.form-group name="title">
                        <x-slot name="labelSlot"><label class="text-white/90">Título</label></x-slot>
                        <input type="text" name="title" value="{{ old('title', $seoPage->title) }}" class="auth-form-input w-full" required>
                    </x-ui.form-group>
                    <x-ui.form-group name="content">
                        <x-slot name="labelSlot"><label class="text-white/90">Contenido (HTML)</label></x-slot>
                        <textarea name="content" rows="8" class="auth-form-input w-full">{{ old('content', $seoPage->content) }}</textarea>
                    </x-ui.form-group>
                    <x-ui.form-group name="product_id">
                        ...
                        <select name="product_id" class="auth-form-input w-full">
                            ...
                            @foreach($products as $p)
                                <option value="{{ $p->id }}" {{ old('product_id', $seoPage->product_id) == $p->id ? 'selected' : '' }}>{{ $p->name }} ({{ $p->key }})</option>
                            @endforeach
                        </select>
                    </x-ui.form-group>
                    <x-ui.form-group name="store_link">
                        <x-slot name="labelSlot"><label class="text-white/90">Enlace tienda</label></x-slot>
                        <input type="url" name="store_link" value="{{ old('store_link', $seoPage->store_link) }}" class="auth-form-input w-full">
                    </x-ui.form-group>
                    <x-ui.form-group name="meta_description">
                        <x-slot name="labelSlot"><label class="text-white/90">Meta descripción</label></x-slot>
                        <textarea name="meta_description" rows="2" class="auth-form-input w-full">{{ old('meta_description', $seoPage->meta_description) }}</textarea>
                    </x-ui.form-group>
                    <div>
                ...
                <div class="mt-6 flex gap-4">
                    <button type="submit" class="glass-button">Guardar</button>

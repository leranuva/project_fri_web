<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-lg sm:text-xl text-white leading-tight">
            {{ __('Crear Nuevo Enlace del Footer') }}
        </h2>
    </x-slot>
        ...
        <x-ui.card glassmorphism="true" padding="lg" class="mb-6">
            <div class="mb-4 sm:mb-6">
                ...
                <h1 class="text-xl sm:text-2xl md:text-3xl font-bold text-white mb-2">
                    Crear Nuevo Enlace del Footer
                </h1>
                <p class="text-xs sm:text-sm md:text-base text-white/80">
                    Completa el formulario para crear un nuevo enlace o red social en el footer
                </p>
            </div>

            <form method="POST" action="{{ route('admin.footer-links.store') }}" class="space-y-6">
                @csrf
                ...
                <div>
                    ...
                    >
                        <option value="link" {{ old('type') === 'link' ? 'selected' : '' }}>Enlace Normal</option>
                        <option value="social" {{ old('type') === 'social' ? 'selected' : '' }}>Red Social</option>
                    </select>
                ...
                <div id="icon_svg_field" style="display: {{ old('type') === 'social' ? 'block' : 'none' }};">
                ...
                <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 sm:gap-3 pt-4 border-t border-white/10">
                    <button type="submit" class="glass-button glass-button-sm sm:glass-button w-full sm:w-auto flex items-center justify-center min-w-0">
                        ...
                        <span class="text-xs sm:text-sm md:text-base truncate">Crear Enlace</span>
                    </button>

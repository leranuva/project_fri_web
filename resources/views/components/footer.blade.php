...
<footer class="footer">
    <div class="footer-content">
        <a href="{{ route('home') }}" class="footer-brand">
            ...
            <span>{{ $footerSection->brand_name }}</span>
        </a>
        ...
        <div class="footer-social">
            @forelse($footerSocials as $social)
                <a href="{{ $social->url }}" class="social-link" {{ str_starts_with($social->url, 'http') ? 'target="_blank" rel="noopener noreferrer"' : '' }}>
                    @if($social->icon_svg)
                        {!! $social->icon_svg !!}
                    @else
    ...
    <div class="footer-copy">
        @if($footerSection->copyright_text)
            {!! str_replace('{{ date(\'Y\') }}', date('Y'), $footerSection->copyright_text) !!}
        @else
            © {{ date('Y') }} {{ $footerSection->brand_name }}. All rights reserved. Built with modern CSS and love.
        @endif
    </div>
</footer>
@endif

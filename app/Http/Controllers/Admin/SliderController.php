...
class SliderController extends Controller
{
    ...
    public function store(Request $request)
    {
        $validated = $request->validate([
            ...
            'image_file' => ['nullable', 'image', 'mimes:jpeg,jpg,png,gif,webp', 'max:5120'], // 5MB
            'video_url' => ['nullable', 'string', 'max:500'],
            'video_file' => ['nullable', 'file', 'mimes:mp4,webm,ogg,mov', 'max:51200'], // 50MB
            ...
            'video_file.mimes' => 'El video debe ser de tipo: mp4, webm, ogg o mov.',
            'video_file.max' => 'El video no debe ser mayor a 50MB.',
        ]);

        // Validar que al menos imagen o video esté presente
        $hasImage = $request->hasFile('image_file') || $request->filled('image_url');
        $hasVideo = $request->hasFile('video_file') || $request->filled('video_url');
        if (!$hasImage && !$hasVideo) {
            return back()->withErrors(['image_url' => 'Debes proporcionar una imagen o un video para el slide.'])->withInput();
        }
        ...
        if ($request->hasFile('video_file')) {
            $videoPath = $request->file('video_file')->store('sliders', 'public');
    ...
    public function update(Request $request, string $id)
    {
        ...
        // Validar que al menos imagen o video esté presente (o mantener los existentes)
        $hasImage = $request->hasFile('image_file') || $request->filled('image_url');
        $hasVideo = $request->hasFile('video_file') || $request->filled('video_url');
        $hasExistingMedia = $slider->image || $slider->video;
        if (!$hasImage && !$hasVideo && !$hasExistingMedia) {
            return back()->withErrors(['image_url' => 'Debes proporcionar una imagen o un video para el slide.'])->withInput();
        }
        ...
        // Determinar la URL del video (solo actualizar si se proporciona una nueva)
        if ($request->hasFile('video_file')) {
            // Eliminar video anterior si es un archivo local
            if ($slider->video && !str_starts_with($slider->video, 'http')) {
                $oldVideoPath = str_replace(asset('storage/'), '', $slider->video);
                $oldVideoPath = str_replace('storage/', '', $oldVideoPath);
                if (Storage::disk('public')->exists($oldVideoPath)) {
                    Storage::disk('public')->delete($oldVideoPath);
                }
            }
            $videoPath = $request->file('video_file')->store('sliders', 'public');
            $slider->video = asset('storage/' . $videoPath);
        } elseif ($request->filled('video_url')) {
            if ($slider->video && !str_starts_with($slider->video, 'http')) {
                $oldVideoPath = str_replace(asset('storage/'), '', $slider->video);
                $oldVideoPath = str_replace('storage/', '', $oldVideoPath);
            ...
            $slider->video = $validated['video_url'];
        }
    ...
    public function destroy(string $id)
    {
        ...
        // Eliminar video si es un archivo local
        if ($slider->video && !str_starts_with($slider->video, 'http')) {
            $videoPath = str_replace(asset('storage/'), '', $slider->video);
